...
$query[] = " SELECT ".
    ...
    $select = " SELECT DISTINCT eCodPiedra, tNombre FROM ($select)N0 ORDER BY tNombre ASC";

    $pf = fopen("../../recepciones.txt","a");
    fwrite($pf,$select."\n\n");
    fclose($pf);
    ...
    {
        $tHTML .= '<li><input type="hidden" id="eCodPiedra'.$i.'" name="piedra['.$i.'][eCodPiedra]" value="'.$r{'eCodPiedra'}.'">'.ucfirst($r{'tNombre'}).' </li>';
        $i++;    
    }
    $tHTML .= '</ul>';
